...
void DataTransformer<Dtype>::CropImage(const Datum& datum,
  ...
  if (datum.encoded()) {
    ...
    if (param_.force_color() || param_.force_gray()) {
      // If force_color then decode in color otherwise decode in gray.
      DLOG(INFO) << "DataTransformer::CropImage DecodeDatumToCVMat";
      cv_img = DecodeDatumToCVMat(datum, param_.force_color());
    } else {
      DLOG(INFO) << "DataTransformer::CropImage DecodeDatumToCVMatNative";
      cv_img = DecodeDatumToCVMatNative(datum);
    }
    // Crop the image.
    cv::Mat crop_img;

    DLOG(INFO) << "DataTransformer::CropImage CropImage [" << bbox.xmin() << ", "  << bbox.ymin() << ", "  << bbox.xmax() << ", "  << bbox.ymax() << "]"; 
    CropImage(cv_img, bbox, &crop_img);
    // Save the image into datum.
    DLOG(INFO) << "DataTransformer::CropImage EncodeCVMatToDatum " << crop_img.size();
  ...
  DLOG(INFO) << "DataTransformer::CropImage ClipBBox";
  ClipBBox(bbox, &clipped_bbox);
  NormalizedBBox scaled_bbox;
  DLOG(INFO) << "DataTransformer::CropImage ScaleBBox";
...
void DataTransformer<Dtype>::CropImage(const AnnotatedDatum& anno_datum,
  ...
  // Crop the datum.
  DLOG(INFO) << "DataTransformer::CropImage Re-CropImage";
  ...
  DLOG(INFO) << "DataTransformer::CropImage TransformAnnotation";
  TransformAnnotation(anno_datum, do_resize, crop_bbox, do_mirror,
...
void DataTransformer<Dtype>::ExpandImage(const Datum& datum,
  ...
  // If datum is encoded, decode and crop the cv::image.
  if (datum.encoded()) {
    //LOG(INFO) << "DataTransformer::ExpandImage image is encoded";
#ifdef USE_OPENCV
    CHECK(!(param_.force_color() && param_.force_gray()))
        << "cannot set both force_color and force_gray";
    cv::Mat cv_img;
  ...
  // Get the bbox dimension.
  DLOG(INFO) << "DataTransformer::ExpandImage get bbox dimension";
  ...
  // Crop the image using bbox.
  DLOG(INFO) << "DataTransformer::ExpandImage crop image by bbox";
...
void DataTransformer<Dtype>::ExpandImage(const AnnotatedDatum& anno_datum,
  ...
  float expand_ratio;
  DLOG(INFO) << "DataTransformer::ExpandImage max_expand_ratio : " << max_expand_ratio;
  caffe_rng_uniform(1, 1.f, max_expand_ratio, &expand_ratio);
  DLOG(INFO) << "DataTransformer::ExpandImage expand_ratio : " << expand_ratio;
  // Expand the datum.
  NormalizedBBox expand_bbox;
...
void DataTransformer<Dtype>::CropImage(const cv::Mat& img,
  ...
  NormalizedBBox clipped_bbox;
  ClipBBox(bbox, &clipped_bbox);
  DLOG(INFO) << "DataTransformer::CropImage ClipBBox [" << clipped_bbox.xmin() << ", "  << clipped_bbox.ymin() << ", "  << clipped_bbox.xmax() << ", "  << clipped_bbox.ymax() << "]"; 
  NormalizedBBox scaled_bbox;
  ScaleBBox(clipped_bbox, img_height, img_width, &scaled_bbox);
  DLOG(INFO) << "DataTransformer::CropImage ScaleBBox [" << scaled_bbox.xmin() << ", "  << scaled_bbox.ymin() << ", "  << scaled_bbox.xmax() << ", "  << scaled_bbox.ymax() << "]"; 

  // Crop the image using bbox.
  int w_off = static_cast<int>(floor(scaled_bbox.xmin()));
  int h_off = static_cast<int>(floor(scaled_bbox.ymin()));
  int width = static_cast<int>(ceil(scaled_bbox.xmax() - scaled_bbox.xmin()));
  int height = static_cast<int>(ceil(scaled_bbox.ymax() - scaled_bbox.ymin()));
  cv::Rect bbox_roi(w_off, h_off, width, height);
  DLOG(INFO) << "DataTransformer::CropImage crop image from " << img.size() << " to " << bbox_roi.size(); 
  

  *crop_img = img(bbox_roi); //.copyTo(*crop_img);
}
...
void DataTransformer<Dtype>::ExpandImage(const cv::Mat& img,
  ...
  // Get the bbox dimension.
  float height = static_cast<float>(img_height * expand_ratio);
  float width = static_cast<float>(img_width * expand_ratio);
  float h_off, w_off;

  //LOG(INFO) << "DataTransformer::ExpandImage caffe_rng_uniform " << static_cast<float>(height - img_height);
  caffe_rng_uniform(1, 0.f, static_cast<float>(height - img_height), &h_off);
  //LOG(INFO) << "DataTransformer::ExpandImage caffe_rng_uniform " << static_cast<float>(width - img_width);
